	...
	for (i = 0; i < RENDERER_AMT; i++) {
		if (*renderers[i] != NULL) {
			...
			for (j = 0; j < queue_count; j++) {
				...
				// Animation
				int animation_count =
					List_count(render_sprite->animations);
...
void animate(Sprite *sprite)
{
	...
	for (int i = 0; i < animation_count; i++) {
		...
		if (animation->time_start == 0 && animation->time_end == 0) {
			...
			animation->time_end = animation->delay
		...
		case ATTR_X:
			sprite->size->x =
				animation->steps[animation->current_step];
			break;
		case ATTR_Y:
			sprite->size->y =
				animation->steps[animation->current_step];
		...
		if (animation->current_step == animation->steps_total) {
			next_node = List_remove(&sprite->animations,
						current_node->id);
			if (!next_node) {
	...
	for (i = 0; i < RENDERER_AMT; i++) {
		if (*renderers[i] != NULL) {
			...
			for (j = 0; j < queue_count; j++) {
				...
				current_node = List_remove(&renderer_current,
							   current_node->id);
			}
		}
	}
